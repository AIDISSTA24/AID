...
class KNXIPInterface:
    ...
    async def _start(self) -> None:
        ...
        if gateway_ip := self.connection_config.gateway_ip:
            gateway_ip = util.validate_ip(gateway_ip, address_name="Gateway IP")
        if local_ip := self.connection_config.local_ip:
            local_ip = util.validate_ip(local_ip, address_name="Local IP")
        ...
        if self.connection_config.connection_type == ConnectionType.ROUTING:
            ...
            self._start_secure_tunnelling_tcp(
                gateway_ip=gateway_ip,
                gateway_port=self.connection_config.gateway_port,
                keyring=keyring,
            )
        else:
            self._start_automatic(local_ip=local_ip, keyring=keyring)
    ...
    async def _start_secure_tunnelling_tcp(
        ...
        if (
            ...
            _gateway = gateway_descriptor or request_description(
    ...
    async def _start_tunnelling_udp(
        ...
        if local_ip is None:
            local_ip = util.get_default_local_ip(gateway_ip)
            if local_ip is None:
    ...
    async def _start_routing(self, local_ip: str | None) -> None:
        ...
        local_ip = local_ip or util.get_default_local_ip()
        if local_ip is None:
            raise XKNXException("No network interface found.")
    ...
    async def send_cemi(self, cemi: CEMIFrame) -> None:
        ...
        return self._interface.send_cemi(cemi)
    ...
    async def gateway_info(self) -> GatewayDescriptor | None:
        ...
        if isinstance(self._interface, _Tunnel):
            return self._interface.request_description()
        return None
...
class KNXIPInterfaceThreaded(KNXIPInterface):
    ...
    async def _await_from_connection_thread(self, coro: Awaitable[T]) -> T:
        ...
        self._main_loop.run_in_executor(None, finished.wait)
        return fut.result()

    async def start(self) -> None:
        """Start KNX/IP interface."""
        return self._await_from_connection_thread(self._start())
    ...
    async def stop(self) -> None:
        ...
        """
        if self._interface is not None:
            self._await_from_connection_thread(self._interface.disconnect())
            self._interface = None
    ...
    async def send_cemi(self, cemi: CEMIFrame) -> None:
        ...
        return self._await_from_connection_thread(self._interface.send_cemi(cemi))
    ...
    async def gateway_info(self) -> GatewayDescriptor | None:
        ...
        if self._gateway_info is not None:
            return self._gateway_info
        if isinstance(self._interface, _Tunnel):
            return self._await_from_connection_thread(
