    ...
    async def async_scan(self) -> AsyncGenerator[GatewayDescriptor, None]:
        ...
        try:
            ...
            if not scan_task.done():
                scan_task.cancel()
            scan_task  # to bubble up exceptions

    async def _scan(
        self, queue: asyncio.Queue[GatewayDescriptor | None] | None = None
    ) -> None:
        """Scan for gateways."""
        _local_ip = self.local_ip or util.get_default_local_ip(
        ...
        local_ip = util.validate_ip(_local_ip)
        ...
        try:
            self._send_search_requests(udp_transport=udp_transport)
