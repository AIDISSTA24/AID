...
class _Tunnel(Interface):
    ...
    async def _reconnect(self) -> None:
        ...
        if self.transport.transport:
            ...
            self.transport.stop()
        asyncio.sleep(self.auto_reconnect_wait)
        if self.connect():
    ...
    async def send_cemi(self, cemi: CEMIFrame) -> None:
        ...
        async with self._send_lock:
            try:
                self._tunnelling_request(raw_cemi)
            finally:
    ...
    async def _tunnelling_request(self, raw_cemi: bytes) -> None:
        ...
        self._send_tunnelling_request(tunnelling_request)
    ...
    async def do_heartbeat(self) -> None:
        ...
        while True:
            try:
                asyncio.sleep(HEARTBEAT_RATE)
                success, _ = self._connectionstate_request()
                if not success:
                    self._do_heartbeat_failed()
    ...
    async def _do_heartbeat_failed(self) -> None:
        ...
        for _heartbeats_failed in range(3):
            success, status = self._connectionstate_request()
            if success:
                return
        # 3 retries failed
        if status is None:
        ...
        async with self._send_lock:
            try:
                ...
                if self._reconnect_task is None or self._reconnect_task.done():
                    self._tunnel_lost()
                self.xknx.connection_manager.connected.wait()
                try:
                    self._tunnelling_request(raw_cemi)
                except TunnellingAckError as err:
                    raise CommunicationError(
