...
class TasmotaDiscovery:
    ...
    async def start_discovery(
        ...
        self._subscribe_discovery_topic(device_discovered, sensors_discovered)

    async def stop_discovery(self) -> None:
        """Stop receiving discovery messages."""
        self._sub_state = self._mqtt_client.subscribe(self._sub_state, {})
    ...
    async def _subscribe_discovery_topic(
            ...
            if discovery_topic_is_device_config(topic):
                ...
                if mac in self._devices and mac in self._sensors:
                    ...
                    if sensors_discovered:
                        sensors_discovered(sensors, mac)
            else:
                if _payload:
                    try:
        ...
        self._sub_state = self._mqtt_client.subscribe(self._sub_state, topics)
    ...
    async def clear_discovery_topic(self, mac: str, discovery_prefix: str) -> None:
        ...
        if device_discovery_topic:
            self._mqtt_client.publish(device_discovery_topic, "", retain=True)
        if sensor_discovery_topic:
            self._mqtt_client.publish(sensor_discovery_topic, "", retain=True)
