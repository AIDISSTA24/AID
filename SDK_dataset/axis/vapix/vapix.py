...
class Vapix:
    ...
    async def initialize_param_cgi(self, preload_data: bool = True) -> None:
        ...
        if not self.light_control and self.params.light_control:
            self._initialize_api_attribute(LightControl, "light_control")
    ...
    async def initialize_event_instances(self) -> None:
        ...
        self._initialize_api_attribute(EventInstances, "event_instances")
    ...
    async def request(
        ...
        **kwargs: dict,
        ...
        try:
            response = self.device.config.session.request(
