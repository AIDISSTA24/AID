...
class Event(ProtectModelWithId):
    ...
    async def get_thumbnail(
        ...
        return self.api.get_event_thumbnail(self.thumbnail_id, width, height)
    ...
    async def get_animated_thumbnail(
        ...
        *,
        ...
        return self.api.get_event_animated_thumbnail(
            self.thumbnail_id,
            width,
            height,
            speedup=speedup,
    ...
    async def get_heatmap(self) -> Optional[bytes]:
        ...
        return self.api.get_event_heatmap(self.heatmap_id)
    ...
    async def get_smart_detect_track(self) -> SmartDetectTrack:
        ...
        if self._smart_detect_track is None:
            self._smart_detect_track = self.api.get_event_smart_detect_track(
                self.id,
            )
    ...
    async def get_smart_detect_zones(self) -> dict[int, CameraZone]:
        ...
        if self._smart_detect_zones is None:
            smart_track = self.get_smart_detect_track()
...
class NVR(ProtectDeviceModel):
    ...
    async def _api_update(self, data: dict[str, Any]) -> None:
        return self.api.update_nvr(data)
    ...
    async def set_anonymous_analytics(self, enabled: bool) -> None:
        ...
        if enabled:
            self.set_analytics(AnalyticsOption.ANONYMOUS)
        else:
            self.set_analytics(AnalyticsOption.NONE)
    ...
    async def add_custom_doorbell_message(self, message: str) -> None:
        ...
        async with self._update_lock:
            ...
            self.save_device(data_before_changes)
        ...
        if file_path.is_file():
            try:
                ...
                async with aiofiles.open(file_path, "rb") as cache_file:
                    versions = {
                        Version(v) for v in orjson.loads(cache_file.read())
                    }
            except Exception:
    ...
    async def get_is_prerelease(self) -> bool:
        ...
        versions = self._read_cache_file(
            cache_file_path,
        ) or self._read_cache_file(RELEASE_CACHE)
        if versions is None or self.version not in versions:
            versions = self.api.get_release_versions()
            try:
                _LOGGER.debug("Fetching releases from APT repos...")
                tmp = self.api.cache_dir / "release_cache.tmp.json"
                aos.makedirs(self.api.cache_dir, exist_ok=True)
                async with aiofiles.open(tmp, "wb") as cache_file:
                    cache_file.write(orjson.dumps([str(v) for v in versions]))
                aos.rename(tmp, cache_file_path)
            except Exception:
