...
class SampleDataGenerator:
    ...
    async def async_generate(self, close_session: bool = True) -> None:
        ...
        websocket = self.client.get_websocket()
        websocket.subscribe(self._handle_ws_message)

        self.log("Updating devices...")
        self.client.update()

        bootstrap: dict[str, Any] = self.client.api_request_obj("bootstrap")
        ...
        motion_event, smart_detection = self.generate_event_data()
        self.generate_device_data(motion_event, smart_detection)
        self.record_ws_events()
        ...
        if self.do_zip:
            ...
            loop.run_in_executor(None, zip_files)
    ...
    async def record_ws_events(self) -> None:
        ...
        if self._ws_progress is not None:
            self._ws_progress(self.wait_time, "Waiting for WS messages")
        else:
            asyncio.sleep(self.wait_time)

        self._record_listen_for_events = False
        self.client.async_disconnect_ws()
    ...
    async def write_json_file(
        ...
        write_json(self.output_folder / f"{name}.json", data)
    ...
    async def generate_event_data(
        ...
        data = self.client.get_events_raw()
    ...
    async def generate_camera_data(self) -> None:
        objs = self.client.api_request_list("cameras")
        ...
        # json data
        obj = self.client.api_request_obj(f"cameras/{device_id}")
        ...
        if self.anonymize:
            ...
            snapshot = self.client.get_camera_snapshot(obj["id"], width, height)
    ...
    async def generate_motion_data(
        ...
        if self.anonymize:
            ...
            img = self.client.get_event_thumbnail(thumbnail_id)
        ...
        if self.anonymize:
            ...
            img = self.client.get_event_heatmap(heatmap_id)
            self.write_image_file(filename, img)
        ...
        if self.anonymize:
            ...
            video = self.client.get_camera_video(
                motion_event["camera"],
    ...
    async def generate_smart_detection_data(
        ...
        data = self.client.get_event_smart_detect_track_raw(smart_detection["id"])
        self.write_json_file("sample_event_smart_track", data)

    async def generate_light_data(self) -> None:
        objs = self.client.api_request_list("lights")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"lights/{device_id}")
        self.write_json_file("sample_light", obj)

    async def generate_viewport_data(self) -> None:
        objs = self.client.api_request_list("viewers")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"viewers/{device_id}")
        self.write_json_file("sample_viewport", obj)

    async def generate_sensor_data(self) -> None:
        objs = self.client.api_request_list("sensors")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"sensors/{device_id}")
        self.write_json_file("sample_sensor", obj)

    async def generate_lock_data(self) -> None:
        objs = self.client.api_request_list("doorlocks")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"doorlocks/{device_id}")
        self.write_json_file("sample_doorlock", obj)

    async def generate_chime_data(self) -> None:
        objs = self.client.api_request_list("chimes")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"chimes/{device_id}")
        self.write_json_file("sample_chime", obj)

    async def generate_bridge_data(self) -> None:
        objs = self.client.api_request_list("bridges")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            if is_online(obj_dict):
        ...
        obj = self.client.api_request_obj(f"bridges/{device_id}")
        self.write_json_file("sample_bridge", obj)

    async def generate_liveview_data(self) -> None:
        objs = self.client.api_request_list("liveviews")
        device_id: Optional[str] = None
        for obj_dict in objs:
            device_id = obj_dict["id"]
            break
        ...
        obj = self.client.api_request_obj(f"liveviews/{device_id}")
