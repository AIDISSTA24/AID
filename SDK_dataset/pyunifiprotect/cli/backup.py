...
async def _newest_event(ctx: BackupContext) -> Event | None:
    ...
    async with db:
        result = db.execute(select(Event).order_by(Event.start_naive.desc()))
...
async def _update_event(ctx: BackupContext, event: d.Event) -> None:
    ...
    async with db:
        result = db.execute(select(Event).where(Event.id == event.id))
...
async def _download_event_thumb(
    ...
    if force and existing_thumb_path:
        ...
        aos.remove(existing_thumb_path)
    ...
    if existing_thumb_path and str(existing_thumb_path) != str(thumb_path):
        ...
        aos.makedirs(thumb_path.parent, exist_ok=True)
        aos.rename(existing_thumb_path, thumb_path)

    if verify and thumb_path.exists() and not _verify_thumbnail(thumb_path):
        _LOGGER.warning(
    ...
    if not thumb_path.exists():
        ...
        if animated:
            thumbnail = ctx.protect.get_event_animated_thumbnail(event_id)
        else:
            thumbnail = ctx.protect.get_event_thumbnail(event_id)
...
async def _download_event_video(
    ...
    if force and existing_event_path:
        ...
        aos.remove(existing_event_path)
    ...
    if existing_event_path and str(existing_event_path) != str(event_path):
        ...
        aos.makedirs(event_path.parent, exist_ok=True)
        aos.rename(existing_event_path, event_path)

    metadata_valid = True
    if verify and event_path.exists():
        valid = False
        if event.end() is not None:
            valid, metadata_valid = _verify_video_file(
                event_path,
    ...
    if not event_path.exists() and event.end() is not None:
        ...
        aos.makedirs(event_path.parent, exist_ok=True)
        camera.get_video(event.start(), event.end(), output_file=event_path)
        downloaded = True

    if (downloaded or not metadata_valid) and event.end() is not None:
        file_context = event.get_file_context(ctx)
        if not _add_metadata(event_path, event.start(), file_context["title"]):
...
async def _download_event(
    ...
    if camera is not None:
        ...
        downloaded = any(asyncio.gather(*downloads))
...
async def _download_events(
    ...
    async with db:
        ...
        count = cast(int, (db.execute(count_query)).scalar())
            ...
            while offset < count:
                ...
                for event in result.unique().scalars():
                    ...
                    if length > ctx.length_cutoff:
                        ...
                        tasks.put(QueuedDownload(task=None, args=[]))
                        continue
...
async def _events(
    ...
    try:
        ...
        if prune and not force:
            _LOGGER.warning("Pruned %s old event(s)", _prune_events(ctx))

        original_start = ctx.start()
        if not force:
            event = _newest_event(ctx)
            if event is not None:
                ctx.start = event.start()

        _LOGGER.warning("Updated %s event(s)", _update_events(ctx))
        ctx.start = original_start
        count, downloaded = _download_events(
            ctx,
            event_types,
            smart_types,
            verify,
