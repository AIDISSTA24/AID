...
class GoogleNestAPI:
    ...
    async def async_get_structures(self) -> list[Structure]:
        ...
        response_data = self._auth.get_json(self._structures_url())
    ...
    async def async_get_structure(self, structure_id: str) -> Structure | None:
        ...
        data = self._auth.get_json(f"{self._structures_url()}/{structure_id}")
        if NAME not in data:
            return None
    ...
    async def async_get_devices(self) -> list[Device]:
        """Return the devices."""
        response_data = self._auth.get_json(self._devices_url())
    ...
    async def async_get_device(self, device_id: str) -> Device | None:
        ...
        data = self._auth.get_json(f"{self._devices_url()}/{device_id}")
        if NAME not in data:
            return None
