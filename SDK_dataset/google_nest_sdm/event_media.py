...
class EventMediaManager:
    ...
    async def _async_load(self) -> OrderedDict[str, EventMediaModelItem]:
        ...
        store_data = self._cache_policy.store.async_load()
        ...
        async with self._lock:
            ...
            for i in range(0, self._cache_policy.event_cache_expire_count()):
                ...
                for media_key in old_item.all_media_keys():
                    self._cache_policy.store.async_remove_media(media_key)
            self._async_update(event_data)
    ...
    async def _fetch_media(self, item: EventMediaModelItem) -> None:
        ...
        if CameraClipPreviewTrait.NAME in self._traits:
            ...
            event_image = clip_preview_trait.generate_event_image(
                clip_event.preview_url
            )
            if event_image:
                content = event_image.contents()
                ...
                store.async_save_media(media_key, content)
                return
        ...
        for event in item.events.values():
            if event.event_id in item.event_media_keys or event.is_expired:
                ...
                continue
            event_image = event_image_trait.generate_image(event.event_id)
            content = event_image.contents(width=SNAPSHOT_WIDTH_PX)
    ...
    async def get_media_from_token(self, event_token: str) -> Media | None:
        ...
        if not (item := self._async_load_item(token.event_session_id)):
        ...
        if not media_key:
            ...
            return None
        contents = self._cache_policy.store.async_load_media(media_key)
        if not contents:
    ...
    async def get_clip_thumbnail_from_token(self, event_token: str) -> Media | None:
        ...
        token = EventToken.decode(event_token)
        if (
            not (item := self._async_load_item(token.event_session_id))
            or not item.visible_event()
        ):
        ...
        if item.thumbnail_media_key:
            ...
            contents = self._cache_policy.store.async_load_media(
        ...
        try:
            self._cache_policy.transcoder.transcode_clip(
        ...
        contents = self._cache_policy.store.async_load_media(thumbnail_media_key)
        if not contents:
    ...
    async def async_handle_events(self, event_message: EventMessage) -> None:
        ...
        for event_session_id, event_dict in event_sessions.items():
            ...
            if model_item := self._async_load_item(event_session_id):
