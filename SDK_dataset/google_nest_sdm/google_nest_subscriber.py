...
class GoogleNestSubscriber:
    ...
    async def create_subscription(self) -> None:
        ...
        try:
            creds = self._auth.async_get_creds()
        except ClientError as err:
            DIAGNOSTICS.increment("create_subscription.creds_error")
            raise AuthException(f"Access token failure: {err}") from err
        try:
            self._subscriber_factory.async_create_subscription(
                creds,
                self._subscriber_id,
    ...
    async def delete_subscription(self) -> None:
        ...
        try:
            self._subscriber_factory.async_delete_subscription(
    ...
    async def start_async(self) -> None:
        ...
        try:
            self._subscriber_future = (
                self._subscriber_factory.async_new_subscriber(
    ...
    async def _watchdog(self) -> None:
        ...
        while True:
            if self._subscriber_future and self._subscriber_future.done():
                ...
                asyncio.sleep(self._watchdog_restart_delay_seconds)
                ...
                self.start_async()
            asyncio.sleep(self._watchdog_check_interval_seconds)
    ...
    async def async_get_device_manager(self) -> DeviceManager:
        ...
        return self._device_manager_task
    ...
    async def _async_message_callback(
        ...
        if (
            ...
            self._device_manager_task.result().async_handle_event(event)
            message.ack()
