...
class RtspStream(Stream):
    ...
    async def extend_stream(self) -> Stream | RtspStream:
        """Extend the lifetime of the stream."""
        return self.extend_rtsp_stream()
    ...
    async def extend_rtsp_stream(self) -> RtspStream:
        ...
        response_data = self.cmd.execute_json(data)
...
class EventImage(CommandModel):
    ...
    async def contents(
        ...
        if width:
            ...
            assert self.url
            fetch_url = self.url
        return self.cmd.fetch_image(fetch_url, basic_auth=self.token)
