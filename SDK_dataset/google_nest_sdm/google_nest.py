...
async def RunTool(args: argparse.Namespace, user_creds: Credentials) -> None:
    ...
    async with ClientSession() as client:
        ...
        if args.command == "list_structures":
            structures: list[Structure] = api.async_get_structures()
            for s in structures:
                PrintStructure(s, args.output_type)
            return

        if args.command == "get_structure":
            structure: Structure | None = api.async_get_structure(
                args.structure_id
            )
            assert structure
        ...
        if args.command == "subscribe":
            ...
            if args.device_id:
                device_manager = subscriber.async_get_device_manager()
        ...
        device: Device | None = api.async_get_device(args.device_id)
        assert device
        ...
        if args.command == "generate_web_rtc_stream":
            ...
            if args.offer_file:
                ...
                offer_sdp = f.read()
            stream = trait.generate_web_rtc_stream(offer_sdp)
