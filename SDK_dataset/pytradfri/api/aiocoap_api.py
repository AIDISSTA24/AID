...
class APIFactory:
    ...
    async def _get_response(
        ...
        try:
            ...
            pr_resp: Message = asyncio.wait_for(pr_req.response, timeout)
    ...
    async def _execute(self, api_command: Command[T], timeout: float | None) -> T:
        ...
        if api_command.observe:
            self._observe(api_command, timeout)
        ...
        _, res = self._get_response(msg, timeout)
    ...
    async def request(
        ...
        if not isinstance(api_commands, list):
            ...
            result = self._execute(api_commands, timeout)
        ...
        command_results: list[T] = asyncio.gather(*commands)
    ...
    async def _observe(self, api_command: Command[T], timeout: float | None) -> None:
        ...
        pr_req, pr_rsp = self._get_response(msg, timeout)
