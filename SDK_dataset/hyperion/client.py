...
class HyperionClient:
    ...
    async def __aenter__(self) -> "HyperionClient" | None:
        ...
        result = self.async_client_connect()
    ...
    async def _call_callbacks(self, command: str, arg: dict[str, Any]) -> None:
        ...
        for callback in callbacks:
            if inspect.iscoroutinefunction(callback):
                cast(Awaitable[None], callback(arg))
            else:
                callback(arg)
    ...
    async def async_client_connect(self) -> bool:
        ...
        try:
            self._reader, self._writer = asyncio.wait_for(
        ...
        self._await_or_stop_task(self._receive_task, stop_task=True)
        self._receive_task = asyncio.create_task(self._receive_task_loop())

        self._client_state_reset()
        self._client_state.update(
        ...
        if not self._raw_connection:
            if (
                ...
                and not self.async_client_login()
            ):
                self.async_client_disconnect()
                return False
            ...
            if (
                ...
                and not self.async_client_switch_instance()
            ):
                self.async_client_disconnect()
                return False
            ...
            ) and not ServerInfoResponseOK(self.async_get_serverinfo()):
                self.async_client_disconnect()
                return False
    ...
    async def async_client_login(self) -> bool:
        ...
        if self._token is None:
            ...
            self._call_client_state_callback_if_necessary()
            return True
        return bool(LoginResponseOK(self.async_login(token=self._token)))
    ...
    async def async_client_switch_instance(self) -> bool:
        ...
        resp_json = self.async_switch_instance(instance=self._target_instance)
        return (
    ...
    async def async_client_disconnect(self) -> bool:
        ...
        self._await_or_stop_task(self._maintenance_task, stop_task=True)
        self._maintenance_task = None

        return self._async_client_disconnect_internal()
    ...
    async def _async_client_disconnect_internal(self) -> bool:
        ...
        self._await_or_stop_task(receive_task, stop_task=True)
        return not error
    ...
    async def _async_safely_read_command(
        ...
        try:
            ...
            resp = asyncio.wait_for(future_resp, timeout=timeout_secs)
        except ConnectionError:
            _LOGGER.warning("Connection to Hyperion lost (%s) ...", self._host_port())
            self._async_client_disconnect_internal()
            return None
        ...
        try:
            while True:
                ...
                if not self._client_state.get(const.KEY_CONNECTED):
                    if not self.async_client_connect():
                        _LOGGER.info(
                        ...
                        self._async_client_disconnect_internal()
                        asyncio.sleep(const.DEFAULT_CONNECTION_RETRY_DELAY_SECS)
                        continue
    ...
    async def _receive_task_loop(self) -> None:
        ...
        while self._async_receive_once():
            pass
    ...
    async def _async_receive_once(self) -> bool:
        ...
        resp_json = self._async_safely_read_command(use_timeout=False)
        if not resp_json:
            return False
        ...
        self._call_client_state_callback_if_necessary()
        self._handle_response_for_caller(resp_json)
        return True
    ...
    class AwaitResponseWrapper:
        """Wrapper an async *send* coroutine and the response."""
        ...
        async def __call__(
            ...
            tan = hyperion_client._reserve_tan_slot(kwargs.get(const.KEY_TAN))
            data = hyperion_client._set_data(kwargs, hard={const.KEY_TAN: tan})
            timeout_secs = self._extract_timeout_secs(hyperion_client, data)

            response = None
            if self._coro(hyperion_client, *args, **data):
                response = hyperion_client._wait_for_tan_response(
                    tan, timeout_secs
                )
            hyperion_client._remove_tan_slot(tan)
            return response
    ...
    async def async_send_is_auth_required(self, *_: Any, **kwargs: Any) -> bool:
        ...
        return self._async_send_json(data)
