...
class Broker:
    ...
    async def _basic_login(self) -> None:
        ...
        if self.refresh_token:
            ...
            try:
                self._obtain_access_token(CREDS_REFRESH_TOKEN | credentials)  # type: ignore[operator]
        ...
        if not self.refresh_token:
            ...
            self._obtain_access_token(CREDS_USER_PASSWORD | self._credentials)  # type: ignore[operator]
    ...
    async def _obtain_access_token(self, credentials: dict[str, int | str]) -> None:
        ...
        try:
            response, content = self._client(  # type: ignore[assignment]
                HTTPMethod.POST,
                AUTH_URL,
